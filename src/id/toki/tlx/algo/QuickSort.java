...
public class QuickSort {
  ...
  public static int partisi(int[] data, int start, int end) {
    ...
    for (int index = start;
        index <= (end - 1);
        index++) { // loop untuk membandingkan element data dengan pivot
        ...
        if (data[index] == data[i] && (index == i)) {
          ...
          System.out.println(
              " : "
                  ...
                  + i
                  + "]");
          int temp = data[i]; // tukar element yang < pivot
    ...
    if (data[end] == data[i + 1]) {
      System.out.println(
          " : end step, "
              + data[end]
              + " tetap (di kiri tidak ada yang > "
              + data[end]
              + ")\n"
              + Arrays.toString(data));
    } else {
      System.out.println(
          " : end step, tukar " + data[end] + " dengan " + data[i + 1] + "[" + (i + 1) + "]");
      int temp = data[i + 1]; // tukar pivot dengan
  ...
  public static void quick(int[] data, int start, int end) { // proses sorting
    if (start < end) {
      counter++;
      System.out.print("\nStep " + counter);
