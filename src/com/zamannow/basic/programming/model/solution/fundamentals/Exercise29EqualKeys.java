...
public class Exercise29EqualKeys {
    ...
    // find the number of element that are smaller than the key
    public static int rank(int key, int[] array) {
        return lessThanKey(key, array, 0, array.length - 1);
    }

    public static int lessThanKey(int key, int[] array, int low, int high) {
        if (low <= high) {
    ...
    public static int greaterThanKey(int key, int[] array, int low, int high) {
        if (low <= high) {
            int middle = low + (high - low) / 2;
    ...
    public static boolean verify(int key, int[] array) {
